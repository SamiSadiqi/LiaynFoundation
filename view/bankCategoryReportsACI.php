...
<div class="page-container">
	...
	<div class="main-container">
		...
		<div class="main-content">
			...
			<div class="row">
				<?php
					if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['bankCategory'])){
						$categoryId = $_POST['bankCategory'];
						$rowCategory = $conn->selectRecord ("tbl_banks_category","id  = ". $categoryId);

					}else{
						
						header("location: banksCategorisReportsACI.php");
						exit();
				...
				<div class="col-lg-6 col-md-6 col-sm-6 col-lg-offset-3 col-md-offset-3 col-sm-offset-3">
					...
					<h3 class="text-center"> All <?php  echo $rowCategory['name'];   ?> Accounts</h3>
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$table  = "tbl_banks";       
						$targetPage = "bankCategoryReportsACI.php";    
						$limit      = 35;  
						if($categoryId !='all'){
							$row_count_sql  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE  deleted = 0 AND category_id = $categoryId ORDER BY id DESC");
						...
						//Get Page Data
						if($categoryId !='all'){
							$requestRawSQLQuery    = "SELECT * FROM $table WHERE deleted = 0 AND category_id = $categoryId  ORDER BY id DESC LIMIT $start, $limit";
						}elseif($categoryId =='all'){
							...
							<thead>
								<tr>
									...
									<th class='text-center'>Currency</th>
 									...
 									<th class="text-center dontPrint">Details</th>
								</tr>
							</thead>
							<tbody>
							<?php
								$counter = $start;
									while($row = $requestSQLQuery->fetch_array()){
										...
										$totalCreditAmount =  $conn->query("SELECT sum(amount) as CreditAmount,sum(home_amount) as creditAmountAFG FROM tbl_bank_statement WHERE  deleted = 0 AND transaction_type =1 AND banks_id = $banksId");
										$fetchTotalCreditAmount    = $totalCreditAmount->fetch_array();
								
										$totalDebitAmount =$conn->query ("SELECT sum(amount) as DebitAmount,sum(home_amount) as debitAmountAfG FROM tbl_bank_statement WHERE deleted = 0 AND transaction_type = 2 AND banks_id = $banksId");
									  ...
									  ?>
										<tr>
											...
											<td class="small text-center dontPrint"><a title="See Details" target="_" href="genralBalanceSheetReport.php?id=<?php echo encryptIt($row['id']);   ?>"><i class="glyphicon glyphicon-send btn btn-blue btn-outline"></i></a></td>
										 </tr>
								<?php 
									}
								?>
