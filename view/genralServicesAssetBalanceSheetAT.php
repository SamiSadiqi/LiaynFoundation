<?php
	require_once("_header.php");
  	$formHeader  = "Genral Services Payment (Income)";
			...
			<div class="row">
				...
				<div class="col-lg-6 col-md-6 col-sm-6 col-lg-offset-3 col-md-offset-3 col-sm-offset-3">
					<h1 class="text-center"><?php echo $companyName; ?></h1>
						...
						if(!empty($fromDate) && !empty($toDate)){
							$title="Genral Information About Assets Services From <b> ".$fromDate."  </b> to <b>".$toDate;
							
						}elseif(empty($fromDate) && empty($toDate)){
							$title="Genral Information About Assets Services";
						}
						?>
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$targetPage = "genralServicesAssetBalanceSheetAT.php";    
						$limit      = 40;  
						if(!empty($fromDate) && !empty($toDate)){
							$rowCountSQL  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE (date BETWEEN '$fromDate' AND '$toDate') AND  deleted = 0 AND transaction_type = 2 ORDER BY id DESC");
						}elseif(empty($fromDate) && empty($toDate)){
 							$rowCountSQL  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE deleted = 0 AND transaction_type = 2 ORDER BY id DESC");
						...
						//Get Page Data
						if(!empty($fromDate) && !empty($toDate)){
							$dataSql    = "SELECT * FROM $table WHERE (date BETWEEN '$fromDate' AND '$toDate') AND deleted = 0 AND transaction_type = 2 ORDER BY id DESC LIMIT $start, $limit";
						}elseif(empty($fromDate) && empty($toDate)){
							$dataSql    = "SELECT * FROM $table WHERE deleted = 0 AND transaction_type = 2 ORDER BY id DESC LIMIT $start, $limit";
						}
					...
					<div class="panel-body table-responsive" id="page-content"> 
    					<div class="form-group dontPrint"> 
        					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4"> 
								...
								<form method="post" action="excelServiceTransactionAssets.php">
						...
						<table  class="table table-bordered table-hover" id ="example" >
							<thead>
								<tr>
									<th class="text-center table-title" colspan="10">
										All Service Payments Assets Transaction
									</th>
								</tr>
							...
							<tbody>
								...
								?>
								<tr>
									<td class='text-center' colspan='7' ><b>Total Assets Services Payment Amounts</b></td>
