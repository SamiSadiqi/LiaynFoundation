<?php
	require_once("_header.php");
 	$formHeader  = "موجودی گدام ها ";
		
		$stockId = $_POST['stockId'];
		$rowStock = $conn->selectRecord ("tbl_stocks","id  = ". $stockId);

		if($stockId == 'all'){
		     $title = "تمام کتگوری ها ";
		}elseif($stockId !='all' ){
		    $title = "<b>".$rowStock['name']."</b> ";
 		}
			...
			<div class="row">
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$targetPage = "stockWarehouseListAT.php";    
						$limit      = 35;  
						if($stockId == "All"){
							...
							<thead>
								<tr>
									<th class="small text-center">شماره</th>
 									<th class='text-center'>نام جنس</th>
							...
							<?php
								$counter = $start;
									while($row = $requestSQLQuery->fetch_array()){
										...
										$selectItemsQuery  = $conn->query("SELECT * FROM  tbl_items WHERE id = $itemsId");
										$fetchItemsData  = $selectItemsQuery->fetch_array();
										$itemName = $fetchItemsData['name'];
