<?php
	require_once("_header.php");
 	$formHeader  = "Customer Catagories";
		
		$categoryId = $_POST['customerCategory'];
		$rowCategory = $conn->selectRecord ("tbl_customer_categories","id  = ". $categoryId);

		if($categoryId == 'all'){
		     $title = "All Customer Categories";
		}elseif($categoryId !='all' ){
		    $title = "<b>".$rowCategory['name']."</b> Category";
 		}
			...
			<div class="row">
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$table  = "tbl_customers";       
						$targetPage = "customerCategoryReportsResultsAT.php";    
						$limit      = 35;  
						if($categoryId == "all"){
							$row_count_sql  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE deleted = 0 ORDER BY id DESC");
						}else{
							$row_count_sql  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE  deleted = 0 AND customer_type = $categoryId ORDER BY id DESC");
						}
						...
						if($categoryId == "all"){
							...
							$requestRawSQLQuery    = "SELECT * FROM $table WHERE deleted = 0 AND customer_type = $categoryId  ORDER BY id DESC LIMIT $start, $limit";
						}
					...
					<div class="panel-body" id="page-content"> 
						<div class="form-group dontPrint"> 
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4"> 
								<form method="post" action="excelReportsCustomerListAT.php">
								...
								$counter = $start;
									while($row = $requestSQLQuery->fetch_array()){
									  ...
									  ?>
										...
										<td class='text-center'><?php echo gmailFormat($row['created_at']);?></td>
										...
										<td class='text-center'><?php echo $row['address']; ?></td> </tr>
								<?php 
									}
								?>
							</tbody>
