<?php
	...
	$menu = "reportsAT";
 	$formHeader  = "Genral Bank Transaction Through Credit";
			...
			<div class="row">
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$targetPage = "genralBankTransactionCreditAT.php";    
						$limit      = 40;  
						if(!empty($fromDate) && !empty($toDate)){
							$rowCountSQL  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE (date BETWEEN '$fromDate' AND '$toDate') AND  deleted = 0 AND transaction_type = 1 AND place='Bank Transaction' ORDER BY id DESC");
						}elseif(empty($fromDate) && empty($toDate)){
 							$rowCountSQL  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE deleted = 0 AND transaction_type = 1 AND place='Bank Transaction'ORDER BY id DESC");
						...
						//Get Page Data
						if(!empty($fromDate) && !empty($toDate)){
							$dataSql    = "SELECT * FROM $table WHERE (date BETWEEN '$fromDate' AND '$toDate') AND deleted = 0 AND transaction_type = 1 AND place='Bank Transaction' ORDER BY id DESC LIMIT $start, $limit";
						}elseif(empty($fromDate) && empty($toDate)){
							$dataSql    = "SELECT * FROM $table WHERE deleted = 0 AND transaction_type = 1 AND place='Bank Transaction' ORDER BY id DESC LIMIT $start, $limit";
						}
					...
					<div class="panel-body table-responsive" id="page-content"> 
    					<div class="form-group dontPrint"> 
        					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4"> 
								...
								<form method="post" action="excelBankCreditedReportsAT.php">
						...
						<table  class="table table-bordered table-hover" id ="example" >
							...
							<tbody>
								...
								?>
								<tr>
									<td class='text-center' colspan='7' ><b>Total Credit Amounts</b></td>
