...
<?php
		if($_SERVER['REQUEST_METHOD'] == "GET" && isset($_GET['id'])){
 		...
 		$vendorPayment = $conn->query("SELECT * FROM  tbl_customer_payment WHERE id = $factorRowId  AND deleted = 0");
		...
		$date = $rowVendorPayment['date'];
 		...
 		$customerId = $rowVendorPayment['customers_id'];
  		$rowVendor = $conn->selectRecord ("tbl_customers","id  =".$rowVendorPayment['customers_id']);
...
<div class="container" style='margin:0 auto;margin-top:20px;height:450px;border:1px solid black;'>
	...
	<div class='row'>
		<table style='width:100%' dir='rtl' border='0'>
			...
			<h6 Style='margin-top:-25px;margin-left:20px;'><b>Customer's Payment</b></h6>

		<hr style='border:1px solid black;margin-top:-5px;'>
		<hr style='border:1px solid black;margin-top:-17px;'>
	</div>
	<?php
	$selectCommingAmount  =$conn->query("select sum(amount)as comingAmount from tbl_customer_statement where deleted='0' AND customers_id='$customerId' AND transaction_type='1'");
 	$rowCommingAmount = $selectCommingAmount->fetch_array();
	$commingAmount = $rowCommingAmount['comingAmount'];
	 
	$selectGoingAmount = $conn->query("select sum(amount)as goingAmount from tbl_customer_statement where deleted='0' AND customers_id='$customerId' AND transaction_type='2'");
	...
	<div class='row' style="height:50px;">
		<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">Customer Name :<?php    echo $fullName; ?></div>
