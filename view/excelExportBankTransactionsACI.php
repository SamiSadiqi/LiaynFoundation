<?php 
	...
	$selectDate    = "SELECT * FROM $table WHERE  deleted = 0 AND  banks_id  = $bankId  ORDER BY id DESC";
	...
	$objPHPExcel->getActiveSheet()->setCellValue("B$i",$row['date']);
 	...
 	$objPHPExcel->getActiveSheet()->setCellValue("F$i",$row['place']);
	$objPHPExcel->getActiveSheet()->setCellValue("G$i",$targetCategory);
	$objPHPExcel->getActiveSheet()->setCellValue("H$i",$targetSubCategory);
	...
	header('Content-Disposition: attachment;filename="Bank_Balance_reports.xls"');
