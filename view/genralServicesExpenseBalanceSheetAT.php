<?php
	require_once("_header.php");
  	$formHeader  = "Genral Services Payment (Expenses)";
			...
			<div class="row">
				...
				<div class="col-lg-6 col-md-6 col-sm-6 col-lg-offset-3 col-md-offset-3 col-sm-offset-3">
					<h1 class="text-center"><?php echo $companyName; ?></h1>
						...
						if(!empty($fromDate) && !empty($toDate)){
							$title="Genral Information About Expense Services From <b> ".$fromDate."  </b> to <b>".$toDate."</b>";
							
						}elseif(empty($fromDate) && empty($toDate)){
							$title="Genral Information About Expense Services";
						}
						?>
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$targetPage = "genralServicesExpenseBalanceSheetAT.php";    
						$limit      = 40;  
						if(!empty($fromDate) && !empty($toDate)){
							$rowCountSQL  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE (date BETWEEN '$fromDate' AND '$toDate') AND  deleted = 0 AND transaction_type = 1 ORDER BY id DESC");
						}elseif(empty($fromDate) && empty($toDate)){
 							$rowCountSQL  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE deleted = 0 AND transaction_type = 1 ORDER BY id DESC");
						...
						//Get Page Data
						if(!empty($fromDate) && !empty($toDate)){
							$dataSql    = "SELECT * FROM $table WHERE (date BETWEEN '$fromDate' AND '$toDate') AND deleted = 0 AND transaction_type = 1 ORDER BY id DESC LIMIT $start, $limit";
						}elseif(empty($fromDate) && empty($toDate)){
							$dataSql    = "SELECT * FROM $table WHERE deleted = 0 AND transaction_type = 1 ORDER BY id DESC LIMIT $start, $limit";
						}
					...
					<div class="panel-body table-responsive" id="page-content"> 
    					<div class="form-group dontPrint"> 
        					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4"> 
								...
								<form method="post" action="excelserviceTransactionExpensesAT.php">
							...
							<thead>
								<tr>
									<th class="text-center table-title" colspan="11">
										All Expenses Service Transaction
									</th>
								</tr>
							...
							<tbody>
								...
								while($serviceProviderRow = $serviceProviderAssests->fetch_array()){
									...
									<tr>
										...
										<td class='text-center'><?php echo $serviceProviderRow['date']; ?></td>
										<td class='text-center'><?php  echo $rowProvider['name']; ?></td>
										<td class='text-center'><?php  echo $serviceProviderRow['factor_number']; ?></td>
										<td class='text-center'><?php  echo $rowCurrency['code']; ?></td>
										<td class='text-center'><?php  echo $serviceProviderRow['amount']; ?></td>
										<td class='text-center'><?php  echo $serviceProviderRow['rate']; ?></td>
										<td class='text-center'><?php  echo $rowBank['name']; ?></td>
										<td class='text-center'><?php  echo $serviceProviderRow['home_amount']; ?></td>
										<td class='text-center'><?php  echo $serviceProviderRow['description']; ?></td>
										<td class="small text-center"><a title="Print Factor" href="printServicePaymentFactorAT.php?id=<?php echo encryptIt($serviceProviderRow['id']); ?>"><i class="fa fa-print btn btn-blue btn-outline"></i></a></td>
									</tr>
									<?php 
									}
									}
									?>
									<tr>
										<td class='text-center' colspan='8' ><b>Total Expense Services Payment Amounts</b></td>
										<td  class='text-center number'><b><?php  echo number_format($totalUsd,2);  ?>  USD</b></td>
									</tr>
							</tbody>
							<?php
