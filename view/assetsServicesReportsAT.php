<?php
	...
	$title = "All Assets Which Serviced in Company";
			...
			<div class="row">
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<?php
						...
						$limit      = 35;  
						if($providerId == "all"){ 
							$row_count_sql  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE  deleted = 0 AND transaction_type = 2 ORDER BY id DESC");
						}elseif($providerId != "all"){
							$row_count_sql  = $conn->query("SELECT COUNT(*) AS totalRows FROM $table WHERE  deleted = 0 AND transaction_type = 2 AND id = $providerId ORDER BY id DESC");
						}
						...
						//Get Page Data
						if($providerId == "all"){ 
							$requestRawSQLQuery    = "SELECT * FROM $table WHERE deleted = 0 AND transaction_type = 2  AND id = $providerId  ORDER BY id DESC LIMIT $start, $limit";
						}elseif($providerId != "all"){
							$requestRawSQLQuery    = "SELECT * FROM $table WHERE deleted = 0 AND transaction_type = 2 ORDER BY id DESC LIMIT $start, $limit";
						}	
