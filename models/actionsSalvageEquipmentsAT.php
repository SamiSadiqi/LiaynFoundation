		...
		$stockId		= $_POST['stockId'];
 		...
 		if($validParam === "expenseEquipmentsAT"){
			...
			for($i=0;$i<sizeof($itemId);$i++){
				if(!empty($itemId[$i])){
					$vendorFactorDetailsSql    = "INSERT INTO `tbl_salvage_equipments`(date,items_id,items_unit_id,amount,stocks_id,description,created_at,users_id, deleted) 
					values('$validDate[$i]','$itemId[$i]','$itemUnit[$i]','$amount[$i]','$stockId[$i]','$subDescription[$i]','$currentTime',$userId, '0')";
					...
					//Get Last Record Id
					$lastQuery = $conn->query("SELECT * FROM `tbl_salvage_equipments` WHERE users_id = $userId AND deleted = 0 ORDER BY id DESC LIMIT 1");
					...
					values('$validDate[$i]','Add Recycling Items','$lastId','1','$amount[$i]','$stockId[$i]','$itemId[$i]','$subDescription[$i]','0','$currentTime','$userId')");
       		...
       		if($addStockBalance && $vendorFactorDetailsQuery && $stockStatement){
				$conn->query("COMMIT");
				header("location: ../view/addSalvageValueAT.php?save");
				exit();
			}else{
				$conn->query("ROLLBACK");
				header("location: ../view/addSalvageValueAT.php?error");
				exit();
			}
